...
def render(viewpoint_camera, pc : Gaussian, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    try:
        ...
        # Backward compatibility for older diff_gaussian_rasterization builds
        # that do not accept the Mip-Splatting specific options.
        raster_kwargs.pop("kernel_size", None)
        raster_kwargs.pop("subpixel_offset", None)
        raster_settings = GaussianRasterizationSettings(**raster_kwargs)
    ...
    if isinstance(raster_out, tuple):
        ...
        radii = torch.zeros(means3D.shape[0], device=means3D.device, dtype=means3D.dtype)
